...
df = pd.DataFrame(data=coords06240910, columns=['X', 'Y', 'Frame', 'Fringes', 'rX', 'rY', 'Angle', 'Volunteer', 'Area'])
...
for index, row in aggregated_data.iterrows():
    if 50 < row['x_crd'] < 150 and 100 < row['y_crd'] < 180:
        antinode1 = antinode1.append(row, ignore_index=True)
    elif 60 < row['x_crd'] < 145 and 333 < row['y_crd'] < 382:
        antinode2 = antinode2.append(row, ignore_index=True)
    elif 150 < row['x_crd'] < 225 and 310 < row['y_crd'] < 370:
        antinode3 = antinode3.append(row, ignore_index=True)
    elif 300 < row['x_crd'] < 370 and 260 < row['y_crd'] < 310:
        antinode4 = antinode4 .append(row, ignore_index=True)
    elif 290 < row['x_crd'] < 370 and 110 < row['y_crd'] < 180:
