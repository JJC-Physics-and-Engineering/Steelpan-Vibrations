...
df = pd.DataFrame(data=coords06241902, columns=['X', 'Y', 'Frame', 'Fringes', 'rX', 'rY', 'Angle', 'Volunteer', 'Area'])
...
for index, row in aggregated_data.iterrows():
    if 170 < row['x_crd'] < 240 and 120 < row['y_crd'] < 200:
        antinode1 = antinode1.append(row, ignore_index=True)
    elif 220 < row['x_crd'] < 280 and 0 < row['y_crd'] < 50:
        antinode2 = antinode2.append(row, ignore_index=True)
    elif 175 < row['x_crd'] < 250 and 275 < row['y_crd'] < 350:
        antinode3 = antinode3.append(row, ignore_index=True)
    elif 0 < row['x_crd'] < 50 and 90 < row['y_crd'] < 220:
        antinode4 = antinode4 .append(row, ignore_index=True)
    elif 0 < row['x_crd'] < 80 and 320 < row['y_crd'] < 382:
